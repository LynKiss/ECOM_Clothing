...
export function useCart() {
  ...
  const addItem = useCallback(
    async (productId: string, quantity: number, variantId?: string, meta?: GuestItemMeta) => {
      if (!session) {
        if (!meta) return;
        const items = loadGuestItems();
      ...
      await clientApi.post('/cart/items', { productId, quantity, variantId });
      await fetchCart();
    },
    [session, fetchCart],
  );
