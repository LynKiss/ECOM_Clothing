...
type ProductVariant = {
  ...
  images?: Array<{ imageId: string; imageUrl: string; sortOrder: number }>;
};
...
export default function Products() {
  ...
  const [quickAddSizeId, setQuickAddSizeId] = useState<string | null>(null);
  ...
  const getActiveVariants = (product: Product) =>
    (product.variants ?? []).filter((variant) => variant.isActive && variant.stockQuantity > 0);
  ...
  const openQuickAdd = (product: Product) => {
    ...
    setQuickAddColorId(first?.color?.colorId ?? null);
    setQuickAddSizeId(first?.size?.sizeId ?? null);
  };
  ...
  const getAvailableColors = (product: Product) => {
    ...
    for (const variant of getActiveVariants(product)) {
      if (variant.color) map.set(variant.color.colorId, variant.color);
    }
    return [...map.values()];
  };

  const getAvailableSizes = (product: Product, colorId: string | null) => {
    const map = new Map<string, ProductSize>();
    for (const variant of getActiveVariants(product)) {
      if (colorId && variant.color?.colorId !== colorId) continue;
      if (variant.size) map.set(variant.size.sizeId, variant.size);
    }
    return [...map.values()].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
  };

  const findQuickAddVariant = (product: Product) => {
    const variants = getActiveVariants(product);
    return variants.find((variant) => {
      const colorOk = quickAddColorId ? variant.color?.colorId === quickAddColorId : true;
      const sizeOk = quickAddSizeId ? variant.size?.sizeId === quickAddSizeId : true;
      return colorOk && sizeOk;
    }) ?? variants[0] ?? null;
  };

  const handleDirectAddVariant = async (product: Product, variantId: string, e: MouseEvent<HTMLButtonElement>) => {
    if (!session) { void navigate('/client/login'); return; }
    triggerCartFlyAnimation(e.currentTarget);
    setAddingId(product.productId);
    try { await addItem(product.productId, 1, variantId); } finally { setAddingId(null); }
  };

  const handleAddToCart = async (product: Product, e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (!session) { void navigate('/client/login'); return; }
    setAddingId(product.productId);
    try { await addItem(product.productId, 1); } finally { setAddingId(null); }
  };

  const confirmQuickAdd = async (e: MouseEvent<HTMLButtonElement>) => {
    if (!quickAddProduct) return;
    if (!session) { void navigate('/client/login'); return; }
    const variant = findQuickAddVariant(quickAddProduct);
    if (!variant) return;
    triggerCartFlyAnimation(e.currentTarget);
    setAddingId(quickAddProduct.productId);
    try {
      await addItem(quickAddProduct.productId, 1, variant.variantId);
    ...
    return (
      <div className="mt-8 flex items-center justify-center gap-2">
        <button disabled={page <= 1} onClick={() => updateParam('page', String(page - 1))}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 bg-white text-[#0B0F19] disabled:opacity-40 hover:border-[#2563EB]">
          <ChevronLeft size={16} />
        </button>
        {pages.map((p, i) =>
          p === '...' ? (
            ...
            <button key={p} onClick={() => updateParam('page', String(p))}
              className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold transition ${p === page ? 'text-white' : 'border border-black/10 bg-white text-[#0B0F19] hover:border-[#2563EB]'}`}
    ...
    <div className="client-surface min-h-[80vh]">
      <div className="mx-auto max-w-7xl px-4 py-8 lg:px-6">
        ...
        <div className="client-card-soft mb-6 grid grid-cols-3 gap-3 p-3 sm:gap-4 sm:p-4">
          {TRUST_BADGES.map((b) => (
            <div key={b.title} className="flex items-center gap-2 sm:gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#2563EB]/8 sm:h-11 sm:w-11">
                <b.icon size={18} style={{ color: '#2563EB' }} />
              </div>
              <div className="min-w-0">
                <p className="truncate text-xs font-black text-[#0B0F19] sm:text-sm">
                  {b.title}
                </p>
        ...
        {recommendations.length > 0 && (
          <div className="client-card-soft mb-6 p-4">
            <div className="mb-3 flex items-center gap-2">
              <Sparkles size={16} className="text-[#2563EB]" />
              <h2 className="text-sm font-black text-[#0B0F19]">AI gợi ý cho bạn</h2>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {recommendations.slice(0, 4).map((item) => (
                <Link
                  ...
                  className="rounded-xl border border-black/5 bg-white p-2 transition hover:border-[#2563EB]/30"
                >
                  <div className="aspect-square overflow-hidden rounded-lg bg-[#F8FAFC]">
                    {item.primaryImageUrl ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center">
                        <Shirt size={24} className="text-[#2563EB]/25" />
                      </div>
                    )}
                  </div>
                  <p className="mt-2 line-clamp-2 text-xs font-bold text-[#0B0F19]">{item.productName}</p>
                  <p className="mt-1 text-xs font-black text-[#2563EB]">{formatPrice(Number(item.effectivePrice ?? item.basePrice))}</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.25em]" style={{ color: '#2563EB' }}>Sản phẩm</p>
            <h1 className="mt-1 text-3xl font-black" style={{ color: '#0B0F19' }}>
        ...
        <form onSubmit={handleSearch} className="mb-4 flex gap-2 lg:hidden">
          <div className="relative flex-1">
            ...
            <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-[#2563EB]"><Search size={16} /></button>
          </div>
        </form>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {activeFilters.map((f) => (
              <button key={f.key} onClick={f.clear}
                className="flex items-center gap-1.5 rounded-full border border-[#2563EB]/30 bg-white px-3 py-1.5 text-xs font-semibold text-[#2563EB] transition hover:bg-[#2563EB]/8">
                <X size={11} /> {f.label}
              </button>
            ))}
          ...
          <aside className={`${filtersOpen ? 'fixed inset-0 z-50 overflow-y-auto bg-white' : 'hidden'} w-full lg:relative lg:block lg:w-56 lg:shrink-0`}>
            {filtersOpen && (
              <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white p-5">
                <h3 className="font-bold text-[#0B0F19]">Bộ lọc</h3>
                ...
                {categoriesExpanded && (
                  <div className="space-y-1">
                    <button onClick={() => { updateMultiple({ categoryIds: '', categoryId: '', subcategoryId: '' }); setFiltersOpen(false); }}
                      className={`w-full rounded-xl px-3 py-2 text-left text-sm font-semibold transition ${selectedCategoryIds.length === 0 ? 'bg-[#2563EB] text-white' : 'text-[#0B0F19] hover:bg-[#2563EB]/8'}`}>
                      Tất cả sản phẩm
                    </button>
                    {categories.map((cat) => (
                      <button key={cat.categoryId}
                        ...
                        className={`flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-semibold transition ${selectedCategoryIds.includes(cat.categoryId) ? 'bg-[#2563EB]/10 text-[#2563EB]' : 'text-[#0B0F19] hover:bg-[#2563EB]/8'}`}>
                        <input
                          type="checkbox"
                          checked={selectedCategoryIds.includes(cat.categoryId)}
                          readOnly
                          className="h-4 w-4 rounded border-[#2563EB]/30 accent-[#2563EB]"
                        />
                        {cat.categoryName}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    <button onClick={() => updateParam('subcategoryId', '')}
                      className={`w-full rounded-xl px-3 py-2 text-left text-sm transition ${!subcategoryId ? 'font-bold text-[#2563EB]' : 'text-gray-500 hover:text-[#0B0F19]'}`}>
                      Tất cả
                    </button>
                    {subcategories.map((sub) => (
                      <button key={sub.subcategoryId}
                        onClick={() => { updateParam('subcategoryId', sub.subcategoryId); setFiltersOpen(false); }}
                        className={`w-full rounded-xl px-3 py-2 text-left text-sm transition ${subcategoryId === sub.subcategoryId ? 'font-bold text-[#2563EB]' : 'text-gray-500 hover:text-[#0B0F19]'}`}>
                        {sub.subcategoryName}
                      </button>
                    ))}
                  </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full rounded-xl border border-black/10 bg-[#F8FAFC] px-3 py-2 text-sm outline-none focus:border-[#2563EB]"
                  />
                  <input
                    ...
                    {origins.map((o) => (
                      <button
                        ...
                        className={`w-full rounded-xl px-3 py-1.5 text-left text-xs transition ${
                          originId === o.originId
                            ? 'bg-[#2563EB]/10 font-bold text-[#2563EB]'
                            : 'text-gray-500 hover:bg-[#2563EB]/5 hover:text-[#0B0F19]'
                        }`}
                      >
                        {o.originName}
                      </button>
                      ...
                      <button
                        ...
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold transition ${
                          tagId === t.tagId
                            ...
                            : 'bg-[#2563EB]/8 text-[#2563EB] hover:bg-[#2563EB]/15'
                        }`}
                      >
                        #{t.tagName}
                      </button>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mb-5 flex items-center justify-between gap-3">
              <button onClick={() => setFiltersOpen(true)}
                className="flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2.5 text-sm font-semibold text-[#0B0F19] transition hover:border-[#2563EB] lg:hidden">
                <Filter size={15} /> Bộ lọc {activeFilters.length > 0 && <span className="flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-black text-white" style={{ background: '#2563EB' }}>{activeFilters.length}</span>}
              </button>
              <div className="ml-auto flex items-center gap-2">
                ...
                <select value={sort} onChange={(e) => updateParam('sort', e.target.value)}
                  className="rounded-full border border-black/10 bg-white px-4 py-2.5 text-sm font-semibold text-[#0B0F19] outline-none transition hover:border-[#2563EB]">
            ...
            {loading ? (
              ...
              <div className="client-card flex flex-col items-center justify-center py-24 text-center">
                <div className="mb-5 flex h-20 w-20 items-center justify-center rounded-full" style={{ background: '#DBEAFE' }}>
                  <Shirt size={36} style={{ color: '#2563EB' }} />
                </div>
                <h3 className="text-lg font-black text-[#0B0F19]">Không tìm thấy sản phẩm</h3>
              ...
              <div className={
                ...
                {products.map((product) => {
                  ...
                  const selColorId = selectedColorIds[product.productId] ?? '';

                  const displayImg = (() => {
                    if (!selColorId || !product.variants?.length) return product.primaryImageUrl;
                    const v = product.variants.find(vv => vv.color?.colorId === selColorId && (vv.images?.length ?? 0) > 0);
                    return v?.images?.[0]?.imageUrl ?? product.primaryImageUrl;
                  })();

                  const quickSizes = (() => {
                    if (!product.variants?.length) return product.sizeOptions ?? [];
                    const seen = new Set<string>();
                    const sizes: ProductSize[] = [];
                    for (const v of product.variants) {
                      if (!v.isActive) continue;
                      if (selColorId && v.color?.colorId !== selColorId) continue;
                      if (v.size && !seen.has(v.size.sizeId)) { seen.add(v.size.sizeId); sizes.push(v.size); }
                    }
                    return sizes.length > 0 ? sizes : (product.sizeOptions ?? []);
                  })();

                  return (
                    <div
                      key={product.productId}
                      className={`client-card-soft group overflow-hidden transition-all duration-300 hover:border-[#2563EB]/30 ${isList ? 'flex flex-row' : 'flex flex-col'}`}
                    >
                      {/* Image area */}
                      <div
                        className={`relative shrink-0 overflow-hidden bg-gray-50 ${isList ? 'w-32 self-stretch sm:w-40 md:w-48' : 'w-full'}`}
                        style={!isList ? { aspectRatio: '1/1' } : undefined}
                      >
                        <Link to={`/client/products/${product.productId}`} className="absolute inset-0 block">
                          <div className="absolute inset-0 flex items-center justify-center" style={{ background: '#F8FAFC' }} aria-hidden="true">
                            <Shirt size={32} className="opacity-20" style={{ color: '#2563EB' }} />
                          </div>
                          {displayImg && (
                            <img
                              src={displayImg}
                        ...
                        <div className={`absolute left-2 top-2 flex flex-col gap-1 ${isList ? '' : 'sm:left-3 sm:top-3 sm:gap-1.5'}`}>
                          {hasDiscount && discountPct > 0 && (
                            <span className="rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-black text-white sm:px-2.5 sm:py-1 sm:text-[11px]">-{discountPct}%</span>
                          )}
                          {!isList && isNewProduct(product.createdAt) && (
                            <span className="inline-flex items-center gap-0.5 rounded-full bg-[#DBEAFE] px-2 py-0.5 text-[10px] font-black text-[#2563EB]"><Sparkles size={9} /> Mới</span>
                          )}
                          {!isList && isBestSeller(product) && (
                            <span className="inline-flex items-center gap-0.5 rounded-full bg-[#F1F5F9] px-2 py-0.5 text-[10px] font-black text-[#0B0F19]"><Award size={9} /> Bán chạy</span>
                          )}
                        </div>
                        ...
                        {!isList && (
                          <button
                            ...
                            className={`absolute right-2 top-2 z-20 flex h-8 w-8 items-center justify-center rounded-full transition-all ${isWishlisted ? 'bg-red-50 text-red-500' : 'bg-white/95 text-gray-400 hover:text-red-400'}`}
                          >
                        ...
                        {outOfStock && (
                          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                            <span className="rounded-full bg-white px-3 py-1 text-[10px] font-black text-gray-800 sm:px-4 sm:py-1.5 sm:text-xs">Hết hàng</span>
                          </div>
                        )}

                        {/* Quick size picker — slides up on hover (grid mode) */}
                        {!isList && !outOfStock && (
                          <div className="absolute bottom-0 left-0 right-0 z-10 translate-y-full opacity-0 transition-all duration-300 ease-out group-hover:translate-y-0 group-hover:opacity-100">
                            <div className="bg-white/96 px-3 py-2.5 backdrop-blur-sm">
                              <p className="mb-2 text-[10px] font-black uppercase tracking-wider text-[#0B0F19]">Thêm nhanh vào giỏ +</p>
                              {quickSizes.length > 0 ? (
                                <div className="flex flex-wrap gap-1.5 pb-0.5">
                                  {quickSizes.map((size) => {
                                    const variant = product.variants?.find(
                                      v => v.isActive && v.stockQuantity > 0 && (!selColorId || v.color?.colorId === selColorId) && v.size?.sizeId === size.sizeId
                                    );
                                    return (
                                      <button
                                        key={size.sizeId}
                                        type="button"
                                        disabled={!variant || addingId === product.productId}
                                        onClick={(e) => { if (variant) void handleDirectAddVariant(product, variant.variantId, e); }}
                                        className={`min-w-[34px] rounded-lg px-2 py-1.5 text-[11px] font-bold transition-colors ${
                                          variant
                                            ? 'border border-black/10 bg-white text-[#0B0F19] hover:border-[#2563EB] hover:bg-[#2563EB] hover:text-white active:scale-95'
                                ...
                                <button
                                  ...
                                  className="w-full rounded-lg bg-[#1D4ED8] py-1.5 text-xs font-bold text-white transition hover:bg-[#1e40af] disabled:opacity-60"
                                >
                                  {addingId === product.productId ? <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-white border-t-transparent" /> : 'Thêm vào giỏ'}
                                </button>
                              )}
                            </div>
                          </div>
                      ...
                      <div className={`flex flex-1 flex-col ${isList ? 'p-3 sm:p-4' : 'p-3'}`}>
                        <div className="flex-1">
                          {product.category && (
                            <p className="mb-0.5 text-[10px] font-bold uppercase tracking-[0.15em]" style={{ color: '#2563EB' }}>
                              {product.category.categoryName}
                            </p>
                          )}
                          <Link
                            to={`/client/products/${product.productId}`}
                            className={`block font-bold leading-snug text-[#0B0F19] transition hover:text-[#2563EB] line-clamp-2 ${isList ? 'text-sm sm:text-base' : 'text-sm'}`}
                          >
                            {product.productName}
                          </Link>
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-2">
                            {rating > 0 && (
                              <div className="flex items-center gap-1">
                                <div className="flex">
                                  {[1, 2, 3, 4, 5].map((s) => (
                                    <Star key={s} size={10} fill={s <= Math.round(rating) ? '#f59e0b' : 'none'} className={s <= Math.round(rating) ? 'text-amber-400' : 'text-gray-200'} />
                                  ))}
                                </div>
                                {product.ratingCount > 0 && <span className="text-[10px] text-gray-400">({product.ratingCount})</span>}
                              </div>
                            )}
                            {product.origin && <span className="text-[10px] text-gray-400">📍 {product.origin.originName}</span>}
                          </div>

                          {/* Color swatches — grid & list */}
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-1.5">
                              {product.colorOptions?.slice(0, 6).map((color) => (
                                <button
                                  key={color.colorId}
                                  type="button"
                                  onClick={() => setSelectedColorIds(prev => ({ ...prev, [product.productId]: prev[product.productId] === color.colorId ? '' : color.colorId }))}
                                  title={color.colorName}
                                  className={`h-5 w-8 rounded-full border-2 transition-transform hover:scale-105 ${selColorId === color.colorId ? 'border-[#2563EB]' : 'border-transparent shadow-[0_0_0_1px_rgba(0,0,0,0.12)]'}`}
                                  style={{ background: color.colorCode ?? '#e5e7eb' }}
                                />
                              ))}
                              {(product.colorOptions?.length ?? 0) > 6 && (
                                <span className="text-[10px] font-bold text-gray-500">+{(product.colorOptions?.length ?? 0) - 6}</span>
                              )}
                            </div>
                          )}
                          ...
                          {isList && (
                            <div className="mt-1.5 flex flex-wrap gap-1">
                              {isNewProduct(product.createdAt) && (
                                <span className="inline-flex items-center gap-0.5 rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-600"><Sparkles size={9} /> Mới</span>
                              )}
                              {isBestSeller(product) && (
                                <span className="inline-flex items-center gap-0.5 rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-600"><Award size={9} /> Bán chạy</span>
                              )}
                              {product.isFeatured && (
                                <span className="rounded-full bg-purple-50 px-2 py-0.5 text-[10px] font-bold text-purple-600">⭐ Nổi bật</span>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div className={`mt-2 ${isList ? 'border-t border-black/5 pt-2 sm:mt-3 sm:pt-3' : 'border-t border-black/5 pt-2'}`}>
                          <div className={isList ? 'mb-2' : ''}>
                            <div className="flex flex-wrap items-baseline gap-1.5">
                              <span className={`font-black ${isList ? 'text-base sm:text-lg' : 'text-base'}`} style={{ color: '#2563EB' }}>
                            ...
                            {lowStock && <p className="mt-0.5 text-[10px] font-semibold text-orange-500">Chỉ còn {product.quantityAvailable} sản phẩm</p>}
                          </div>

                          {/* Buttons: LIST MODE ONLY */}
                          {isList && (
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                style={{ background: outOfStock ? '#9ca3af' : '#1D4ED8' }}
                              >
                                {addingId === product.productId ? (
                                  <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white border-t-transparent" />
                                ) : (
                                  <><ShoppingCart size={12} /><span>Thêm vào giỏ</span></>
                                )}
                              </button>
                            </div>
                          )}
            ...
            <div ref={loadMoreRef} className="mt-8 flex min-h-10 items-center justify-center">
              {loadingMore ? (
                <span className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-[#0B0F19]">
      ...
      {quickAddProduct && (() => {
        const colorOptions = getAvailableColors(quickAddProduct);
        const sizeOptions = getAvailableSizes(quickAddProduct, quickAddColorId);
        const selectedVariant = findQuickAddVariant(quickAddProduct);
        const variantImage = selectedVariant?.images?.[0]?.imageUrl;
        const base = Number(selectedVariant?.price ?? quickAddProduct.basePrice);
        const effective = Number(selectedVariant?.salePrice ?? selectedVariant?.price ?? quickAddProduct.effectivePrice);
        const hasDiscount = effective < base - 0.01;
        return (
          <div
          ...
          >
            <div
              className="client-card w-full max-w-xl overflow-hidden"
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-black/5 p-5">
                ...
                <button
                  type="button"
                  onClick={() => setQuickAddProduct(null)}
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 text-gray-500 transition hover:text-[#0B0F19]"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="grid gap-5 p-5 sm:grid-cols-[160px_1fr]">
                <div className="relative aspect-square overflow-hidden rounded-2xl bg-gray-50">
                  {variantImage || quickAddProduct.primaryImageUrl ? (
                    <img
                      src={variantImage ?? quickAddProduct.primaryImageUrl ?? ''}
                      alt={quickAddProduct.productName}
                      className="h-full w-full object-cover"
                    />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center">
                      <Shirt size={42} className="opacity-20" style={{ color: '#2563EB' }} />
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex flex-wrap items-baseline gap-2">
                      <span className="text-xl font-black" style={{ color: '#2563EB' }}>{formatPrice(effective)}</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {colorOptions.map((color) => (
                          <button
                            ...
                            title={color.colorName}
                            onClick={() => {
                              const nextSizes = getAvailableSizes(quickAddProduct, color.colorId);
                              setQuickAddColorId(color.colorId);
                              setQuickAddSizeId((current) => nextSizes.some((size) => size.sizeId === current) ? current : nextSizes[0]?.sizeId ?? null);
                            }}
                            className={`h-9 w-12 rounded-full border-2 transition ${quickAddColorId === color.colorId ? 'border-[#2563EB] ring-2 ring-[#2563EB]/15' : 'border-white shadow-[0_0_0_1px_rgba(0,0,0,0.14)]'}`}
                            style={{ background: color.colorCode ?? '#e5e7eb' }}
                          />
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {sizeOptions.map((size) => (
                          <button
                            key={size.sizeId}
                            type="button"
                            onClick={() => setQuickAddSizeId(size.sizeId)}
                            className={`min-w-14 rounded-xl px-4 py-2 text-sm font-black transition ${quickAddSizeId === size.sizeId ? 'bg-[#0B0F19] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                          >
                            {size.sizeCode || size.sizeName}
                          </button>
                        ))}
                  ...
                  <button
                    type="button"
                    disabled={!selectedVariant || addingId === quickAddProduct.productId}
                    onClick={(event) => void confirmQuickAdd(event)}
                    className="client-pill-primary flex w-full items-center justify-center gap-2 py-3 text-sm font-black disabled:opacity-50"
                  >
                    {addingId === quickAddProduct.productId ? (
            ...
            >
              ...
              <div className="grid sm:grid-cols-2">
                <div className="relative h-64 sm:h-full bg-gray-50">
                  {p.primaryImageUrl ? (
                    ...
                    <div className="flex h-full items-center justify-center" style={{ background: '#F8FAFC' }}>
                      <Shirt size={48} className="opacity-20" style={{ color: '#2563EB' }} />
                    </div>
                  )}
                  {hasDiscount && discountPct > 0 && (
                ...
                <div className="flex flex-col p-6">
                  {p.category && (
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: '#2563EB' }}>
                      {p.category.categoryName}
                    </p>
                  )}
                  <h2 className="mt-2 text-xl font-black" style={{ color: '#0B0F19' }}>
                    {p.productName}
                  </h2>
