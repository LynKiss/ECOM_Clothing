...
import { MapPin, Plus, Check, ChevronRight, ArrowLeft, Shirt, Truck, BadgePercent, CheckCircle2, Tag } from 'lucide-react';
...
export default function Checkout() {
  ...
  useEffect(() => {
    if (!session) {
      // Guest: chỉ cần lấy delivery methods (public endpoint)
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 lg:px-6">
            ...
            <div key={step.label} className="flex items-center gap-2">
              ...
              <span
                className={`text-sm font-semibold ${
                  step.done ? 'text-[#2563EB]' : step.active ? 'text-[#0B0F19] font-black' : 'text-gray-400'
                }`}
              >
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          <div className="space-y-6">
            {/* Addresses */}
            <div>
              <h2 className="mb-4 flex items-center gap-2 text-lg font-black text-[#0B0F19]">
                <MapPin size={20} className="text-[#2563EB]" /> Địa chỉ giao hàng
              </h2>

              {!session ? (
                <div className="client-card border-2 border-[#2563EB] p-5">
                  <p className="mb-1 text-xs font-bold uppercase tracking-wider text-[#2563EB]">Đặt hàng với tư cách khách</p>
                  ...
                  <p className="mt-3 text-xs text-gray-400">
                    Đã có tài khoản?{' '}
                    <a href="/client/login" className="font-semibold text-[#2563EB] hover:underline">Đăng nhập</a>{' '}
                ...
                <div className="space-y-3">
                    ...
                    <button
                      ...
                      className={`w-full rounded-xl border-2 p-4 text-left transition ${
                        selectedAddressId === addr.id && !addingAddress
                          ...
                          : 'border-transparent bg-white hover:border-[#2563EB]/30'
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-bold text-[#0B0F19]">{addr.recipientName}</p>
                            {addr.isDefault && (
                              <span className="rounded-full bg-[#2563EB]/10 px-2 py-0.5 text-[10px] font-black text-[#2563EB]">
                                Mặc định
                              </span>
                            )}
                            {addr.label && (
                  ...
                  {!addingAddress ? (
                    <button
                      ...
                      className="flex w-full items-center gap-2 rounded-xl border-2 border-dashed border-[#2563EB]/20 bg-white px-5 py-4 text-sm font-semibold text-[#2563EB] transition hover:border-[#2563EB]/40"
                    >
                      <Plus size={16} /> Thêm địa chỉ mới
                    </button>
                  ) : (
                    <div className="client-card border-2 border-[#2563EB] p-5">
                      <h3 className="mb-4 font-bold text-[#0B0F19]">Địa chỉ mới</h3>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {[
                          { key: 'recipientName', label: 'Họ và tên *', placeholder: 'Nguyễn Văn A' },
                          { key: 'phone', label: 'Số điện thoại *', placeholder: '0901234567' },
            ...
            {deliveryMethods.length > 0 && (
              <div>
                <h2 className="mb-4 flex items-center gap-2 text-lg font-black text-[#0B0F19]">
                  <Truck size={20} className="text-[#2563EB]" /> Phương thức vận chuyển
                </h2>
                <div className="space-y-3">
                  {deliveryMethods.map((dm) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-bold text-[#0B0F19]">{dm.name}</p>
                          {dm.description && (
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`font-black text-sm ${Number(dm.basePrice) === 0 ? 'text-[#2563EB]' : 'text-[#0B0F19]'}`}>
                            {Number(dm.basePrice) === 0 ? 'Miễn phí' : formatPrice(Number(dm.basePrice))}
                          </span>
                          <div
                            className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition ${
                              selectedDeliveryId === dm.id ? 'border-[#2563EB] bg-[#2563EB]' : 'border-gray-300'
                            }`}
                          >
            ...
            <div className="flex gap-3">
              <Link
                to="/client/cart"
                className="client-pill-dark-outline flex items-center gap-2 px-5 py-3 text-sm font-semibold transition hover:border-[#2563EB] hover:text-[#2563EB]"
              >
          ...
          <div className="space-y-4">
            <div className="client-card p-5">
              <p className="mb-3 flex items-center gap-2 text-sm font-black text-[#0B0F19]">
                <Tag size={15} /> Voucher cho đơn này
              </p>

              {appliedDiscountCode ? (
                <div className="mb-3 rounded-xl border border-[#2563EB]/15 bg-[#DBEAFE]/45 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="flex items-center gap-2 text-sm font-black text-[#2563EB]">
                        <CheckCircle2 size={16} /> {appliedDiscountCode}
                      </p>
                      <p className="mt-1 text-xs font-semibold text-[#0B0F19]/80">
              ...
              <div className="mt-4 space-y-2">
                {loadingVouchers ? (
                  <div className="rounded-xl bg-[#F1F5F9] px-4 py-5 text-center text-xs font-semibold text-gray-500">
                    Đang gợi ý voucher...
                  </div>
            ...
            <div className="client-card p-5">
              ...
              <div className="max-h-64 space-y-3 overflow-y-auto">
                {cart.items.map((item) => (
                  <div key={item.id} className="flex items-start gap-3">
                    <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl bg-[#F8FAFC]">
                      {item.primaryImageUrl ? (
                        ...
                        <div className="flex h-full items-center justify-center">
                          <Shirt size={18} className="text-[#2563EB]/30" />
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="line-clamp-2 text-xs font-semibold text-[#0B0F19]">
                        {item.productName}
                      </p>
                      <p className="text-xs text-gray-400">x{item.quantity}</p>
                    </div>
                    <p className="shrink-0 text-xs font-bold text-[#2563EB]">
              ...
              <div className="mt-4 space-y-2 border-t border-black/5 pt-4 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-semibold ${shipping === 0 ? 'text-[#2563EB]' : ''}`}>
                    {selectedDelivery
                ...
                <div className="flex justify-between border-t border-black/5 pt-2 text-base">
                  <span className="font-black text-[#0B0F19]">Tổng cộng</span>
                  <span className="font-black text-[#2563EB]">{formatPrice(total)}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div
      className={`rounded-xl border p-3 ${
        selected
          ? 'border-[#2563EB] bg-[#DBEAFE]/45'
          : eligible
            ? 'border-[#2563EB]/15 bg-white'
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-sm font-black text-[#0B0F19]">{voucher.code}</p>
              <p className="line-clamp-1 text-xs font-semibold text-gray-500">
                {voucher.name}
              </p>
            </div>
            <span className="rounded-full bg-[#DBEAFE] px-2 py-1 text-[11px] font-black text-[#2563EB]">
