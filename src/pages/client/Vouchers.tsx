  ...
  return (
    <div className="client-surface min-h-[80vh]">
      <div className="mx-auto max-w-6xl px-4 py-10 lg:px-6">
        ...
        {!session ? (
          <div className="mt-6 rounded-xl border border-[#2563EB]/15 bg-[#DBEAFE]/45 p-5 text-sm font-semibold text-[#0B0F19]">
              ...
              <button
                ...
                className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-black transition ${
                  filter === item.key
                    ...
                    : 'border border-[#1D4ED8]/25 bg-white text-[#0B0F19] hover:bg-[#DBEAFE]'
                }`}
              >
                {item.label}
              </button>
        ...
        {loading ? (
          ...
          <div className="client-card mt-6 flex flex-col items-center justify-center py-20 text-center">
            <TicketPercent size={56} className="mb-4 text-[#2563EB]/20" />
...
function VoucherHuntCard({
  ...
  return (
    <article className="client-card client-card-hover group overflow-hidden">
      <div className="relative min-h-40 overflow-hidden bg-[#0B0F19] p-5 text-white">
        <div className="relative flex items-start justify-between">
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-white/12">
            <BadgePercent size={28} />
          </div>
          <span className="rounded-full bg-[#DBEAFE] px-3 py-1 text-sm font-black text-[#2563EB]">
      ...
      <div className="p-5">
        ...
        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between text-xs font-bold">
            ...
            <span className={eligible ? 'text-[#2563EB]' : 'text-[#8a5a00]'}>
        ...
        {voucher.isPrivate ? (
          <div className="mt-4 flex items-center gap-2 rounded-xl bg-[#DBEAFE] px-4 py-3 text-xs font-bold text-[#2563EB]">
        ...
        <div className="mt-5 flex gap-2">
          <button
            type="button"
            onClick={onCopy}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-black/10 text-[#0B0F19] transition hover:border-[#2563EB]/30 hover:text-[#2563EB]"
            title="Sao chép mã"
          >
          ...
          <button
            ...
            className="client-pill-outline flex min-h-11 flex-1 items-center justify-center gap-2 px-4 py-3 text-sm font-black disabled:border-[#2563EB]/20 disabled:bg-[#DBEAFE]/50 disabled:text-[#2563EB]"
          >
            {voucher.isSaved ? (
              <>
...
function InfoBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-[#F1F5F9] px-4 py-3">
      ...
      <p className="mt-1 text-sm font-black text-[#0B0F19]">{value}</p>
    </div>
  );
}
