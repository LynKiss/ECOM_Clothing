  ...
  return (
    <div className="client-surface min-h-[80vh]">
      ...
      {article.titleImageUrl && (
        <div className="relative h-72 overflow-hidden md:h-[420px]" style={{ background: '#0B0F19' }}>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
        <Link to="/client/news" className="mb-6 flex items-center gap-2 text-sm font-semibold text-[#2563EB] hover:underline">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_300px]">
          ...
          <div className="space-y-6">
            <article className="client-card p-8 md:p-10">
              ...
              <h1 className="text-3xl font-black leading-tight text-[#0B0F19] md:text-4xl">{article.title}</h1>
              {article.subTitle && (
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-black/5 pt-6">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-black/10 px-4 py-2 text-sm font-semibold text-gray-500 transition hover:border-[#2563EB] hover:text-[#2563EB]"
                >
            ...
            <section className="client-card p-8 md:p-10">
              <h2 className="mb-6 flex items-center gap-2 text-lg font-black text-[#0B0F19]">
              ...
              {session ? (
                ...
                <div className="mb-6 rounded-2xl border border-[#2563EB]/15 bg-[#2563EB]/5 px-5 py-4 text-sm text-[#2563EB]">
              ...
              {commentsLoading ? (
                <div className="flex justify-center py-8">
                  <LoaderCircle size={20} className="animate-spin text-[#2563EB]/40" />
                </div>
              ) : comments.length === 0 ? (
                      ...
                      <div key={c.id} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-2">
                            <p className="text-sm font-bold text-[#0B0F19]">{c.author.username}</p>
                          ...
                          <div className="mt-2 flex items-center gap-2">
                            <button
                              ...
                              className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-semibold transition ${
                                vote === 'like' ? 'border-[#2563EB] bg-[#2563EB]/10 text-[#2563EB]' : 'border-black/10 text-gray-400 hover:text-[#2563EB]'
                              }`}
                            >
                              <ThumbsUp size={11} />
          ...
          <aside className="space-y-4">
                ...
                <div className="space-y-4">
                  {related.map((r) => (
                    <Link key={r._id} to={`/client/news/${r.slug}`} className="group flex items-start gap-3">
                      <div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-[#DBEAFE]">
                        {r.titleImageUrl ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="line-clamp-2 text-sm font-semibold leading-snug text-[#0B0F19] group-hover:text-[#2563EB]">{r.title}</p>
                        <p className="mt-1 text-[11px] text-gray-400">{new Date(r.createdAt).toLocaleDateString('vi-VN')}</p>
                      </div>
                    </Link>
                  ))}
                </div>
                <Link to="/client/news" className="mt-4 flex items-center gap-1 text-xs font-bold text-[#2563EB] hover:underline">
            ...
            <div className="client-feature-band p-5 text-center">
              ...
              <p className="mt-1 text-xs text-white/60">Cập nhật gợi ý phối đồ và xu hướng thời trang mỗi tuần.</p>
              <input
                type="email"
