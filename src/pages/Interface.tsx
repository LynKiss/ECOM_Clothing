...
const DEFAULT_STATS: Array<{ value: string; label: string }> = [
  { value: '15.000+', label: 'khách hàng tin chọn' },
...
const DEFAULT_WHY_US: Array<{ title: string; description: string }> = [
  ...
  { title: 'Hỗ trợ kỹ thuật', description: 'Đội ngũ tư vấn thời trang hỗ trợ chọn size, phối đồ và đổi trả. Hotline miễn phí 1800 6863, hỗ trợ 7 ngày/tuần.' },
...
function ContentEditModal({
  ...
  const [hero, setHero] = useState({
    title: config.hero?.title ?? 'Mỗi phong cách bắt đầu từ lựa chọn đúng.',
    subtitle: config.hero?.subtitle ?? 'Cung cấp quần áo, phụ kiện và outfit chọn lọc theo size, màu, chất liệu — chính hãng, giá tốt, giao nhanh toàn quốc.',
    buttonText: config.hero?.buttonText ?? 'Khám phá ngay',
    bgColor: config.hero?.bgColor ?? '#0B0F19',
  });
            ...
            <>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input className={inputCls + ' flex-1'} value={hero.bgColor} onChange={(e) => setHero((h) => ({ ...h, bgColor: e.target.value }))} placeholder="#0B0F19" />
                </div>
              </div>
            </>
          )}
...
export default function Interface() {
  ...
  const loadAdvancedTheme = () => {
    ...
    return {
      ...
      bgColor: '#F5F1E8',
      borderRadius: 16,
      brandName: 'Fashion Ledger',
      logoUrl: '',
      footerText: '© 2025 Fashion Ledger. Mọi quyền được bảo lưu.',
      hotline: '1800 6863',
      sticky: true,
    };
  };
